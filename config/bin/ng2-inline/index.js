...
module.exports = function (content, options, relativeDir) {
	options = options || {};
	options.base = options.base || './';

	content = processStyleUrls(content, options, relativeDir);
	content = processTemplateUrl(content, options, relativeDir);

	return content;
};

function processStyleUrls(content, options, relativeDir) {
	...
	matches.forEach(function () {
		...
		var result = urls.map(function (url) {
			let file = fs.readFileSync(path.join(relativeDir, url), 'utf-8');
...
function processTemplateUrl(content, options, relativeDir) {
